...
void set_variable(char *varname, char *value)
...
{
    if (tgt->ident != NULL) free((void*)tgt->ident);

    if (tgt->dev != NULL) free((void*)tgt->dev);
    if (tgt->dir != NULL) free((void*)tgt->dir);
    if (tgt->fstype != NULL) free((void*)tgt->fstype);
    if (tgt->mountoptions != NULL) free((void*)tgt->mountoptions);
    if (tgt->fsckoptions != NULL) free((void*)tgt->fsckoptions);
    if (tgt->loopdev != NULL) free((void*)tgt->loopdev);

    if (tgt->cipher != NULL) free((void*)tgt->cipher);

    if (tgt->key.format != NULL) free((void*)tgt->key.format);
    if (tgt->key.filename != NULL) free((void*)tgt->key.filename);
    if (tgt->key.digestalg != NULL) free((void*)tgt->key.digestalg);
    if (tgt->key.cipheralg != NULL) free((void*)tgt->key.cipheralg);
...
#define OFFSET(x) (int)((char*)&((tgtdefn_t*)NULL)->x - (char*)NULL)
    struct tokinfo_t toktable[] = {
        ...
        { "fsoptions",      OFFSET(mountoptions),
                            proc_string,    NULL },  /* Deprecated since 4.1 */
