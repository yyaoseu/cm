...
cm_string_t *cm_str_init(char *val)
 ...
 */
int cm_generate_key(unsigned char *buff, size_t len)
{   struct rnddev {
        const char *name;
        short devmaj;
        short devmin; } *rndsrc;
    ...
    unsigned char *mdval, *pool, *devbuff = NULL;
    ...
    pool = (unsigned char*)sec_realloc(NULL, POOL_SIZE);

    devbuff = (unsigned char*)sec_realloc(NULL, NOISE_CHUNK);
...
{   ssize_t pwlen=0;
    ...
    if (echook) {
        ...
        *buff = sec_realloc((void*)*buff, (size_t)(pwlen+1));
        strcpy(*buff, tmppass);
        mem_cleanse((unsigned char*)tmppass, sizeof(tmppass));
    ...
    *buff = sec_realloc((void*)*buff, (size_t)(pwlen+1));
    strcpy(*buff, tmppass);
    mem_cleanse((unsigned char*)tmppass, (size_t)(pwlen+1));
#endif  /* !HAVE_TERMIOS */
...
{   unsigned char *cptr, buff[64], mrk=0x80;
    ...
    *mdlen = CM_SHA1_SIZE;
    cptr = *mdval;
    for (idx=0; idx<5; ++idx) {
        cptr[0] = (unsigned char)((ctxt->H[idx] >> 24) & 0xff);
        cptr[1] = (unsigned char)((ctxt->H[idx] >> 16) & 0xff);
        cptr[2] = (unsigned char)((ctxt->H[idx] >> 8) & 0xff);
        cptr[3] = (unsigned char)(ctxt->H[idx] & 0xff);
        cptr += 4;
    }
}
...
{   cm_sha1_ctxt_t *mdcontext=NULL;
    unsigned char q, *permsalt=NULL, *mdval=NULL, *mdval_prev=NULL;
    size_t idx, pos, mdlen, pwlen, sz=0, newidx;
    uint32_t cnt, r_val;
    const uint32_t r_mult=421, r_inc=54773, r_mod=259200, r_scale=69317;
    ...
    if (salt != NULL && saltlen > 0) {
        permsalt = sec_realloc((void*)permsalt, saltlen);
    ...
    *key = (unsigned char*)sec_realloc((void*)*key, keylen);
    pos = 0;
    while (pos < keylen) {
        ...
        for (cnt=0; cnt<iterations; ++cnt) {
            ...
            /* Mix-in password: */
            cm_sha1_block(mdcontext, (const unsigned char*)passwd, pwlen);
...
{   unsigned idx, pos, r=0;
    ...
    for (idx=0; tcases[idx].passwd!=NULL; ++idx) {
        cm_pwd_fortify(tcases[idx].passwd, n_its,
                    (unsigned char*)tcases[idx].salt, strlen(tcases[idx].salt),
                    &key, keylen);
#if 0
