 ...
 *  (C)Copyright 2005-2014, RW Penney
 */
...
/** Identifiers of top-level operating mode */
typedef enum {
    M_UNSET, M_HELP,
    M_PREPARE, M_RELEASE,
    M_MOUNT, M_UNMOUNT,
...
/**
 *  Configuration switches set (perhaps indirectly)
 *  from command-line options
 */
enum {
    F_NEEDS_TGT =   1,    /*!< command requires a filesystem target */
    F_ALL_TARGETS = 2,    /*!< command will be applied to all known targets */
    F_VERIFY_PW =   4     /*!< encryption password should be double-checked */
};
...
cmmode_t parse_options(int argc, char *argv[],
                       unsigned *flags, const char **mode_params,
...
{   cmmode_t mode = M_UNSET;
    ...
    *spos = '\0';
    ...
    if (mode == M_UNSET) mode = get_defaultmode(argc, argv);
...
{   cmmode_t mode = M_UNSET;
    ...
    mode = parse_options(argc, argv, &mode_flags, &mode_params,
    ...
    if ((mode == M_LIST || mode == M_STATUS) && optind >= argc) {
        mode_flags |= F_ALL_TARGETS;
    }

    /* if '--all' given, assemble list of targets from entire config-file */
    if ((mode_flags & F_ALL_TARGETS) != 0) {
        if (optind < argc) {
    ...
    if ((mode_flags & F_NEEDS_TGT) && eltlist == NULL) {
