 ...
 *  (C)Copyright 2007-2014, RW Penney
 */
...
static const unsigned char kmblti_version = (char)1;
...
static int kmblti_checkversion(unsigned char fversion)
{
...
static cm_bf_ctxt_t *kmblti_initcipher_v0(const unsigned char *salt,
...
static cm_bf_ctxt_t *kmblti_initcipher_v1(const unsigned char *salt,
...
{   const keyinfo_t *keyinfo = &boundtgt->tgt->key;
    cm_bf_ctxt_t *ctxt;
    enum { BUFFSZ = 512 };
    unsigned char *hbuff=NULL, fversion, salt[kmblti_saltlen],
            *buff=NULL, *bptr;
    ...
    *key = NULL; *keylen = 0;
    ...
    /* Read key header: */
    hbuff = (unsigned char*)malloc((size_t)(kmblti_maglen + 4));
    hbuff[0] = '\0';
...
{   cm_bf_ctxt_t *ctxt;
    unsigned char fversion, salt[kmblti_saltlen], hbuff[4],
                *buff=NULL, *bptr;
