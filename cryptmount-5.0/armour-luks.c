...
/*!
 *  Change the password associated with a given LUKS key-slot.
 *
...
{   tgtdefn_t *tgt = boundtgt->tgt;
    ...
    *key = (unsigned char*)sec_realloc((void*)*key, lcs_keylen);
    ...
    /* Store or create key in LUKS header */
{
    ...
    if (formatting) {
        ...
        if (kmluks_change_slot_passwd(luks_ctxt, keyslot,
                                      key, keylen, passwd) < 0) {
            char errmsg[256];
            crypt_last_error(luks_ctxt, errmsg, sizeof(errmsg));
